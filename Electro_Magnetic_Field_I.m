...
function[Ex,Ey,Ez,Bx,By,Bz,x,y,z,dx,dt,iterations,c2,noe,Tx,Ty,Tz]=Electro_Magnetic_Field_I(length,nofe,c,totaltime,disturbance_time,A,w,draw_disturbance,Plot_or_not,scale,rotate,az_user,el_user,Pause_or_not)
            %Create mesh
...
for n=2:iterations
        ...
        Ez1=Ez(:,:,:,n-1);
    %Magnetic Field
    [Bx2]=CURL_E_Bx_I(c,noe,Bx1,Ez1,Ey1);
    [By2]=CURL_E_By_I(c,noe,By1,Ex1,Ez1);
    [Bz2]=CURL_E_Bz_I(c,noe,Bz1,Ey1,Ex1);
        %Electric Field
    [Ex2]=CURL_B_Ex_I(c,noe,Ex1,Bz2,By2);
    [Ey2]=CURL_B_Ey_I(c,noe,Ey1,Bx2,Bz2);
    [Ez2]=CURL_B_Ez_I(c,noe,Ez1,By2,Bx2);
        %Update Magnetic field
        Bx(:,:,:,n)=Bx2;
        By(:,:,:,n)=By2;
        Bz(:,:,:,n)=Bz2;
...
%Pseudo field
Ex_P=zeros(noe,iterations);
Ey_P=zeros(noe,iterations);
Ez_P=zeros(noe,iterations);
%Pseudo length
for i=1:noe
        x_P(i,1)=x(i,i,i);
        y_P(i,1)=y(i,i,i);
